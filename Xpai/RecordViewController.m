...
//    [XpaiInterface setAuthMode:CLEAR_PASSWORD];
    ...
    if ([SettingConfig sharedInstance]._connectionMode == CONNECT_CLOUD) {//直播云
        [SettingConfig sharedInstance]._getCloudVSUrl = ip.text;
...
- (IBAction)logoutButtonPressed:(id)sender //退出按钮
{
...
- (IBAction)uploadPressed:(id)sender
{
    ...
    NSLog(@"新文件上传 : %@",self.currentFileName);
...
- (IBAction)resumeLiveButtonPressed:(id)sender
{
    ...
    NSLog(@"断电续传 :%@",self.currentFileName);
    NSLog(@"id :%@",self.currentStreamId);
    NSLog(@"path %@",self.currentStreamPath);
}
