...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        NSArray *array1 = [[NSArray alloc] initWithObjects:@"拍摄Demo",@"拍摄参数设置",@"拍摄NewDemo", nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            ...
            case 2:{
                NewRecordViewController * newrecordView = [[NewRecordViewController alloc]init];
                MainNaVi * newRecordNV = [[MainNaVi alloc]initWithRootViewController:newrecordView];
                [self presentViewController:newRecordNV animated:YES completion:^{
